...
class PoseEstimator:
    ...
    def infer(
        ...
        bboxes_np = (
            np.array(bboxes_array, dtype=np.float32)
            if bboxes_array
            else np.array([[0, 0, width - 1, height - 1]], dtype=np.float32)
        )
        ...
        for sample in pose_data_samples:
            ...
            scores = (
                preds.keypoint_scores if hasattr(preds, "keypoint_scores") else None
            )
            ...
            if (
                hasattr(preds, "bboxes")
                and preds.bboxes is not None
                and len(preds.bboxes) > 0
            ):
    ...
    def infer_batch(
        ...
        for i, frame_bgr in enumerate(frames):
            bboxes = (
                person_bboxes_list[i]
                if person_bboxes_list and i < len(person_bboxes_list)
                else None
            )
            frame_results = self.infer(
                frame_bgr, person_bboxes=bboxes, auto_detect=auto_detect
            )
